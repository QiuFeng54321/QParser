...
public class TrieState : State
{
    ...
    private int _acceptedTokenType;
    private char _fromChar;
    private bool _isAccept;

    public TrieState(State? parentState, char fromChar, bool canBeIdentifier,
        int acceptedTokenType = TokenConstants.Unknown) :
        base(parentState)
    {
        _fromChar = fromChar;
    ...
    ///     There exists a custom transition from any trie node
    ///     on the path generated by <paramref name="str" />, possibly by calling
    ///     <see cref="Add(string,QParser.Lexer.States.State)" />
    /// </exception>
    public void Add(string str, int tokenType)
    {
        State cur = this;
    ...
    public void Add(string str, State state)
    {
        ...
        for (var index = 0; index < str.Length; index++)
        {
            ...
            else if (!curTrie._next.ContainsKey(c))
            {
                curTrie._next[c] = new TrieState(cur, c, canBeIdentifier);
            }
    ...
    {
        return new Token(_acceptedTokenType, str);
    }

    public override StateTransition MakeTransitionToThis()
    {
        return new StateTransition(this,
            ParentState is not RootState ? StateTransitionFlag.ConsumeChar : StateTransitionFlag.None);
    }
